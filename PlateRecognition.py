...
class PlateRecognition():
    ...
    def VLPR(self, car_pic):
        ...
        print(" card_imgs, colors", card_imgs, colors)
        # car_pic = cv2.imread(car_pic)
        # card_imgs = detect5.detect(car_pic)
...
def cutVideo(videoFileName,f):
    i = 0
    video = cv2.VideoCapture(videoFileName)  # 读取视频文件
    while (True):
        ret, frame = video.read()
        cv2.imshow('video', frame)    #实时显示视频播放
        c = cv2.waitKey(10)
        if c == 27:
            break
        i = i + 1
        ...
        # if i % 24 == 0:                   #除数为间隔的帧数
        #     cv.imwrite('D:/pycharm/demo1/demoimage/' + str(i) + '.jpg', frame)
        #     print(i)

        if i == f:  # f为具体的某一帧数
            cv2.imwrite('./Test/videoframe/' + str(i) + '.jpg', frame)
            # print(i)
            videoframe = './Test/videoframe/'+str(i)+'.jpg'
...
if __name__ == '__main__':
    ...
    c = PlateRecognition()
    t1 = time.time()
    result = c.VLPR("D:/LPDsystem/img/1.jpg")
